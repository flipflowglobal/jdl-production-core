// Recovered EVM analysis engine. The full disasm → CFG → symbolic-execution →
// decompiler pipeline is wired into the public `analyze_bytecode` entry point, so
// the recovered code is genuinely reachable and no blanket dead_code suppression is
// needed. Remaining pub items form the module's public API.
